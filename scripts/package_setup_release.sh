...
#
# Writes: dist/silent-bomber-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix silent-bomber \
  --exe-name Silent_Bomber_Recompiled \
  --display-name "Silent Bomber Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --runtime-dir mods \
  --disc-hint "Silent Bomber (Europe).cue" \
